...
from shared.logging.redaction import redact
...
class NyraLogger:
    ...
    def emit(self,event:str,kind:LogKind=LogKind.EVENT,level:LogLevel=LogLevel.INFO,payload:Any=None,**params):
        rec=LogRecord(ct=self.ct,event=event,kind=kind,level=level,payload=redact(payload),params=redact(params),**self.context); return self.emit_record(rec)
    def emit_record(self,record:LogRecord):
        clean=record.model_copy(update={"params":redact(record.params),"payload":redact(record.payload)})
        self.q.put_nowait(clean); return clean
