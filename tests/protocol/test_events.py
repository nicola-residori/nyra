...
def test_interaction_state_contract_serializes_correlation():
    event = InteractionStateChanged(state=InteractionState.LLM_REASONING, source={"id":"speaker-a","area":"living-room"}, session_id=generate_session_id(), request_id=generate_request_id(), trace_id=generate_trace_id())
    data = event.model_dump(mode="json")
    assert data["event"] == "INTERACTION_STATE_CHANGED"
    assert data["category"] == "interaction_state"
    assert data["state"] == "LLM_REASONING"


def test_session_closed_contract_has_stable_reason():
    event = SessionClosedEvent(session_id=generate_session_id(), request_id=generate_request_id(), trace_id=generate_trace_id(), close_reason=CloseReason.DIRECT_COMMAND)
...
def test_subscription_and_snapshot_contracts():
    ...
    snap = StateSnapshot(source_id="speaker-a", state=InteractionState.PROCESSING)
...
def test_events_reject_unprefixed_correlation_ids():
    ...
    from shared.protocol.events import InteractionStateChanged, InteractionState
